const CACHE_NAME = 'aakash-ai-v11';
const BASE = '/aakash-ai/';
const ASSETS = [
  BASE, BASE+'index.html', BASE+'icons.js', BASE+'crypto.js', BASE+'state.js',
  BASE+'tools.js', BASE+'providers.js', BASE+'ai.js', BASE+'chat.js',
...
self.addEventListener('fetch', event => {
  if (event.request.url.includes('api.anthropic.com') || event.request.url.includes('generativelanguage.googleapis.com') || event.request.url.includes('fonts.googleapis.com') || event.request.url.includes('fonts.gstatic.com') || event.request.url.includes('cdnjs.cloudflare.com')) {
