...
const TOOLS = [
  ...
  // ── WEALTH FEATURES ──
  { name: 'add_income', description: 'Add income when user mentions earning/receiving money. Example: "5000 freelancing se aaye", "salary aa gayi 22000".',
    input_schema: { type:'object', properties: { amount:{ type:'number' }, source:{ type:'string', enum:['Freelancing','Job Salary','Business','Side Hustle','Investment Returns','YouTube/Content','Teaching','Affiliate','Other'] }, description:{ type:'string' } }, required:['amount','source'] } },

  { name: 'add_income_stream', description: 'Add/create an income stream. Example: "naya income source — tutoring", "freelancing stream add kar".',
    input_schema: { type:'object', properties: { name:{ type:'string' }, status:{ type:'string', enum:['Active','Building','Planned','Paused'] }, revenue:{ type:'number', description:'Monthly revenue' } }, required:['name'] } },

  { name: 'add_financial_goal', description: 'Create a financial goal with target. Example: "mujhe 1 lakh monthly kamana hai December tak".',
    input_schema: { type:'object', properties: { title:{ type:'string' }, target:{ type:'number' }, current:{ type:'number' }, deadline:{ type:'string' }, milestones:{ type:'string', description:'Comma-separated amounts' } }, required:['title','target'] } },

  { name: 'add_skill', description: 'Track a skill for monetization. Example: "Python skill add kar", "meri web dev skill track kar".',
    input_schema: { type:'object', properties: { name:{ type:'string' }, level:{ type:'string', enum:['Beginner','Intermediate','Advanced','Expert'] }, monetization:{ type:'string', enum:['Not yet','Exploring','Earning'] }, linkedStream:{ type:'string' } }, required:['name'] } },

  { name: 'add_business_idea', description: 'Save a business idea. Example: "ek idea hai — AI tutoring app", "business idea save kar".',
    input_schema: { type:'object', properties: { title:{ type:'string' }, oneLiner:{ type:'string' }, problem:{ type:'string' }, potentialRevenue:{ type:'string' }, investment:{ type:'string' }, status:{ type:'string', enum:['Just an idea','Researching','Validating','Building','Launched'] }, priority:{ type:'string', enum:['Low','Medium','High'] } }, required:['title'] } },

  { name: 'get_wealth_summary', description: 'Get complete wealth overview — income, expenses, net worth, streams, goals, skills. Use when user asks "mera financial status", "kitna kamaya".',
    input_schema: { type:'object', properties: {} } },

  { name: 'save_life_lesson', description: 'Save a life lesson, quote, moral, or past mistake that user shares. Example: "mene seekha ki patience zaruri hai", "ek galti ki thi maine — bina sochey invest kiya", "quote save kar — consistency beats talent". Use when user shares wisdom, mistakes, learnings, or quotes they want to remember.',
    input_schema: { type:'object', properties: {
      text:{ type:'string', description:'The lesson/quote/moral/mistake text' },
      type:{ type:'string', enum:['quote','moral','mistake','lesson'], description:'Type of entry' },
      context:{ type:'string', description:'Context or situation when this was learned (optional)' }
    }, required:['text','type'] } }
];
...
function executeTool(name, input) {
  ...
  switch (name) {
    ...
    // ── WEALTH TOOL HANDLERS ──

    case 'add_income': {
      if (!S.finance.income) S.finance.income = [];
      S.finance.income.push({ id:Date.now(), amount:input.amount, source:input.source, desc:input.description||'', date:new Date().toISOString() });
      saveAll();
      return `✅ Income ₹${input.amount} (${input.source}) added.${input.description?' — '+input.description:''}`;
    }

    case 'add_income_stream': {
      if (!S.finance.incomeStreams) S.finance.incomeStreams = [];
      S.finance.incomeStreams.push({ id:Date.now(), name:input.name, status:input.status||'Active', revenue:input.revenue||0, startDate:new Date().toISOString(), notes:'' });
      saveAll();
      const active = S.finance.incomeStreams.filter(s=>s.status==='Active').length;
      return `✅ Stream "${input.name}" added (${input.status||'Active'})! Active streams: ${active}/7 target.`;
    }

    case 'add_financial_goal': {
      if (!S.finance.financialGoals) S.finance.financialGoals = [];
      const milestones = input.milestones ? input.milestones.split(',').map(m => ({ amount:parseFloat(m.trim()), done:false })).filter(m=>m.amount>0) : [];
      S.finance.financialGoals.push({ id:Date.now(), title:input.title, target:input.target, current:input.current||0, deadline:input.deadline||null, milestones, actions:'' });
      saveAll();
      return `✅ Financial goal: "${input.title}" (₹${input.current||0}/₹${input.target}).${input.deadline?' Deadline: '+input.deadline:''}${milestones.length?' Milestones: '+milestones.length:''}`;
    }

    case 'add_skill': {
      if (!S.skills) S.skills = [];
      S.skills.push({ id:Date.now(), name:input.name, level:input.level||'Beginner', monetization:input.monetization||'Not yet', revenue:0, linkedStream:input.linkedStream||'', nextMilestone:'' });
      saveAll();
      return `✅ Skill "${input.name}" tracked (${input.level||'Beginner'}, ${input.monetization||'Not yet'}).`;
    }

    case 'add_business_idea': {
      if (!S.businessIdeas) S.businessIdeas = [];
      S.businessIdeas.push({ id:Date.now(), title:input.title, oneLiner:input.oneLiner||'', problem:input.problem||'', potentialRevenue:input.potentialRevenue||'', investment:input.investment||'', skillsNeeded:'', status:input.status||'Just an idea', priority:input.priority||'Medium', createdAt:new Date().toISOString() });
      saveAll();
      return `✅ Business idea: "${input.title}" saved (${input.status||'Just an idea'}, ${input.priority||'Medium'} priority).`;
    }
    ...
    case 'get_wealth_summary': {
      ...
      return [
        ...
        `🏦 Net Worth: ₹${totalA-totalL} (Assets:₹${totalA}, Liabilities:₹${totalL})`,
        `📊 Income Streams: ${activeStreams.length}/7 active — ${activeStreams.map(s=>s.name+':₹'+s.revenue).join(', ')||'none'}`,
        ...
        `💡 Business Ideas: ${ideas.length} saved`
      ].join('\n');
    }

    case 'save_life_lesson': {
      if (!S.lifeLessons) S.lifeLessons = [];
      S.lifeLessons.push({
        id: Date.now(),
      ...
      if (typeof FIRE !== 'undefined' && FIRE.ready) {
        FIRE.save('lifeLessons', Date.now().toString(), S.lifeLessons[S.lifeLessons.length-1]);
      }
      const emoji = {quote:'💬',moral:'🧭',mistake:'⚠️',lesson:'📝'}[input.type] || '📝';
      return `${emoji} Life lesson saved! (Total: ${S.lifeLessons.length}). Jab zarurat hogi yaad dilaaunga.`;
    }
