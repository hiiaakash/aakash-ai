...
window.pTap = async function(d) {
  ...
  if (_pm === 'setup') { _ps = pin; _pm = 'confirm'; document.getElementById('pL').textContent = 'Confirm PIN'; document.querySelectorAll('.dt').forEach(d => d.classList.remove('on')); }
    ...
    if (await hashPin(pin) === localStorage.getItem('ak_pin_hash')) { _ck = await CR.dk(pin); await loadAll(); render(); setTimeout(()=>{window.runNotificationEngine?.();if(typeof FIRE!=='undefined')FIRE.onLogin();},1500); }
...
// ── AUTO-SAVE: Prevent chat loss on refresh/close ──
window.addEventListener('beforeunload', () => {
  if (S.chat.length > 0) { saveChatToHistory(); }
  // Sync save (can't await in beforeunload)
  if (_ck) {
    try { localStorage.setItem('ak_emergency', JSON.stringify({ chat: S.chat, activeChat: S.activeChat, chats: S.chats })); } catch {}
  }
});

// Periodic auto-save every 30 seconds
setInterval(() => {
  if (_ck && S.chat.length > 0) { saveChatToHistory(); saveAll(); }
}, 30000);

// Recover emergency save on load
setTimeout(() => {
  try {
    ...
    if (em && _ck) {
      ...
      if (d.chat && d.chat.length > 0 && S.chat.length === 0) {
        S.chat = d.chat;
        S.activeChat = d.activeChat;
        if (d.chats && d.chats.length > S.chats.length) S.chats = d.chats;
        saveAll(); render();
        console.log('[APP] Recovered chat from emergency save');
      }
