...
function rMain(app) {
  ...
  <div style="padding:10px 14px 6px;flex-shrink:0;background:var(--c1);border-bottom:1px solid var(--b1);z-index:10">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
      ...
      <div id="onlineStatus" style="padding:2px 8px;border-radius:10px;font-size:9px;font-weight:600;background:${navigator.onLine?'var(--gBg)':'var(--rBg)'};color:${navigator.onLine?'var(--g)':'var(--r)'};border:1px solid ${navigator.onLine?'var(--gBorder)':'var(--rBorder)'}">${navigator.onLine?'Online':'Offline'}</div>
...
// ── Online/Offline indicator (Change 11) ──
window.addEventListener('online', () => { const el=document.getElementById('onlineStatus'); if(el){el.textContent='Online';el.style.background='var(--gBg)';el.style.color='var(--g)';el.style.borderColor='var(--gBorder)';} showToast('Online — API active'); });
window.addEventListener('offline', () => { const el=document.getElementById('onlineStatus'); if(el){el.textContent='Offline';el.style.background='var(--rBg)';el.style.color='var(--r)';el.style.borderColor='var(--rBorder)';} showToast('Offline — Brain mode'); });
