...
function render() {
  ...
  if (!S.apiKey && !S.geminiKey && !(S.apiKeys||[]).some(k => k.enabled)) { rApi(app); return; }
  rMain(app);
}
...
function rApi(app) {
  ...
  <p style="font-size:14px;color:var(--t3);margin-bottom:24px;text-align:center;max-width:300px;line-height:1.7">API key add karo — naam do, key paste karo. Features automatic unlock honge!</p>
  <div style="width:100%;margin-bottom:12px"><label style="font-size:13px;color:var(--t2);font-weight:600;display:block;margin-bottom:6px">Key ka naam</label>
  <input id="setupName" class="inp" placeholder="e.g., Claude, Gemini, Voice wali..." style="font-size:14px"></div>
  <div style="width:100%;margin-bottom:12px"><label style="font-size:13px;color:var(--t2);font-weight:600;display:block;margin-bottom:6px">API Key</label>
  <input id="setupKey" class="inp" placeholder="Paste your API key..." style="font-family:IBM Plex Mono,monospace;font-size:14px">
  <div id="setupDetect" style="font-size:11px;color:var(--t4);margin-top:4px;min-height:16px"></div></div>
  <button onclick="_setupAddKey()" class="btn bp" style="width:100%;padding:14px;font-size:16px">Start AAKASH AI →</button>
  <div style="margin-top:16px;padding:12px;background:var(--c2);border-radius:8px;border:1px solid var(--b1);width:100%;font-size:12px;color:var(--t3);line-height:1.7">📍 Data stored in your browser (localStorage), encrypted. Nothing sent anywhere except AI calls.<br><br>💡 Baad mein Settings se aur keys add kar sakte ho — image, voice, video sab unlock honge!</div></div>`;
  // Auto-detect on input
  document.getElementById('setupKey')?.addEventListener('input', function() {
    const info = document.getElementById('setupDetect');
    if (!info) return;
    const key = this.value.trim();
    if (key.length < 6) { info.textContent = ''; return; }
    const pid = detectProvider(key);
    if (pid === 'unknown') { info.innerHTML = '❓ Provider detect nahi hua — but add kar sakte ho'; info.style.color = 'var(--t4)'; return; }
    const prov = PROVIDER_MAP[pid];
    info.innerHTML = prov.icon + ' <strong>' + prov.name + '</strong> detected! Features: ' + prov.caps.slice(0,4).join(', ');
