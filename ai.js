...
async function ai(messages, sys, onStream) {
  ...
  for (const pid of chatPriority) {
    ...
    try {
      ...
      errors.push(`${(PROVIDER_MAP[pid]?.name||pid)}: ${result.error || 'Unknown error'}`);
    } catch (e) {
      errors.push(`${(PROVIDER_MAP[pid]?.name||pid)}: ${e.message || 'Network/crash error'}`);
      console.log(`Provider ${pid} error:`, e);
    }
  }
  if (errors.length === 0) return 'Koi enabled API key nahi mili. Settings > API Keys mein check karo.';
  return `API Error Details:\n${errors.map(e => '• ' + e).join('\n')}\n\nInternet check karo ya Settings mein keys verify karo.`;
}
