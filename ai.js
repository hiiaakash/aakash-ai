...
//  Smart Auto-Select: Haiku 4.5 / Sonnet 4.6 / Opus 4.6
...
// ── SMART MODEL AUTO-SELECT ──
// Chhota message = Haiku 4.5 (fast, cheap)
// Medium message = Sonnet 4.6 (balanced)
// Deep/complex = Opus 4.6 (most powerful)
// Deep Think toggle overrides to Opus always
function selectModel(messages) {
  // Deep Think ON = always Opus
  ...
  if (typeof lastUser.content === 'string') {
    ...
    // If has image/document, use Sonnet minimum (multimodal needs smarts)
    const hasFile = lastUser.content.some(c => c.type === 'image' || c.type === 'document');
    if (hasFile) return 'claude-sonnet-4-6';
  ...
  // Quick greetings & short messages → Haiku (fast)
  if (len < 30 || words <= 5) return 'claude-haiku-4-5-20251001';

  // Deep/complex indicators → Opus
  const complexPatterns = [
    /explain|samjhao|analysis|analyze|compare|difference|architecture|design|strategy/i,
    /code|function|algorithm|debug|error|implement|build|create.*app/i,
    /essay|article|write.*detailed|research|in.?depth/i,
    /math|calcul|equation|formula|theorem|proof/i,
    /business.*plan|financial.*model|investment/i,
  ];
  const isComplex = complexPatterns.some(p => p.test(text));
  if (isComplex || words > 80) return 'claude-opus-4-6';
...
// ── CLAUDE FALLBACK (if selected model fails) ──
