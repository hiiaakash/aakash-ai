...
async function ai(messages, sys, onStream) {
  ...
  // ── BRAIN-powered provider routing ──
  ...
  for (const pid of chatPriority) {
    ...
    // Brain check — skip providers in cooldown
    if (!BRAIN.isUp(pid)) {
    ...
    tried++;
    let model = null;
    let maxRetries = 3; // max model retries per provider
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        switch (pid) {
          case 'claude':
            model = model || BRAIN.pickModel('claude', selectModel(messages));
            opts.model = model;
        ...
        if (result.ok) {
          BRAIN.markUp(pid, model); // Mark as healthy
          return result.text;
        }

        // Failed — let BRAIN decide what to do
        const decision = BRAIN.handleError(pid, model, result.status || 0, result.error || '');

        if (decision.retry && decision.nextModel) {
          model = decision.nextModel; // retry with next model
          console.log(`[BRAIN] ${pid}: retrying with ${model}...`);
          continue;
        }
        ...
        const decision = BRAIN.handleError(pid, model, 0, e.message || 'crash');
        errors.push(`${(PROVIDER_MAP[pid]?.name||pid)}: ${e.message || 'Network error'}`);
        console.log(`[BRAIN] ${pid} crash:`, e);
        break;
      }
    }
  }
  ...
  if (tried === 0) {
    // All providers in cooldown — check if any cooldown expired
    BRAIN.load(); // refresh state
    const status = BRAIN.getStatus();
    return `Sab providers cooldown mein hain. Thodi der baad try karo.\n\n${status}\n\nYa Settings mein naya key add karo (Groq free hai: console.groq.com)`;
  }
