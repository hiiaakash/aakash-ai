...
async function ai(messages, sys, onStream) {
  ...
  if (typeof MIND !== 'undefined' && MIND.ready && userText) {
    ...
    if (decision.source === 'memory' && decision.answer && decision.confidence >= 0.45) {
      console.log(`[MIND] Answered from MEMORY (${(decision.confidence*100).toFixed(0)}%) — API call saved!`);
      return decision.answer;
    }
  }
