...
REPLY LENGTH:
- Casual baat: 1-2 lines, warm
- Normal baat: 2-3 lines with proper answer
- Explain karna ho: 5-8 lines with examples
- Detail maanga: 10+ lines with thorough explanation
- KABHI unnecessary chhota ya incomplete reply mat do
- Har question ka PROPER answer do — shortcut nahi
...
async function ai(messages, sys, onStream) {
  ...
  if (!S.apiKeys || S.apiKeys.length === 0) {
    ...
    if (!S.apiKeys?.length) {
      ...
      return await _tryBrainFallback(userText, 'API key add karein Settings mein.');
    }
  }
