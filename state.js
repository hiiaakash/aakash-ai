// ════════════════════════════════════
//  AAKASH AI v3 — State (state.js)
//  Data + Multi-chat + Themes + Embedded AI Chat
//  UPDATED: New fields for all v3 features
// ════════════════════════════════════

// ── MAIN STATE ──
let S = {
  // ── User Identity (Change 8) ──
  userPhone: '',                    // Phone number as User ID (permanent)
  userName: '',                     // User's display name
  userJoinedAt: '',                 // First signup date
  ...
  // ── Accountability ──
  accountability: { enabled:false, intensity:'funny' },
  ...
  notifSettings: {
    ...
    habitReminder: { enabled:true },
    financeNudge: { enabled:true },
    goalDeadline: { enabled:true },
    dailyWisdom: { enabled:true },
    accountabilityRoast: { enabled:true },
    milestoneAlert: { enabled:true }
  },
  ...
  // ── Connect System (Change 9) ──
  connect: {
    bots: [
      { id: 1, name: 'Arjun', role: 'career', expertise: 'Career Mentor — career guidance, resume, interview prep', chatHistory: [], enabled: true },
      { id: 2, name: 'Kavya', role: 'life', expertise: 'Life Coach — motivation, mindset, life decisions', chatHistory: [], enabled: true },
      { id: 3, name: 'Rehan', role: 'study', expertise: 'Study Buddy — padhai, concepts, doubt solving', chatHistory: [], enabled: true },
      { id: 4, name: 'Priya', role: 'business', expertise: 'Business Strategist — ideas, execution, market analysis', chatHistory: [], enabled: true },
      { id: 5, name: 'Dev', role: 'fitness', expertise: 'Fitness Coach — workout, diet, discipline', chatHistory: [], enabled: true }
    ],
    partnerCode: '',                // User's own connect code
    connectedPartner: '',           // Partner's code
    partnerActive: false,           // On/Off toggle
    partnerMessages: []             // Temporary — deleted on OFF
  },

  // ── Files Manager (Change 29) ──
  uploadedFiles: [],                // {id, name, type, size, uploadedAt, brainStored:bool}

  // ── Achievements (Change 23) ──
  achievements: {
    badges: [],                     // {id, name, icon, unlockedAt}
    points: 0,
    streaks: { chat:0, habits:0, finance:0 }
  },

  // ── Daily Challenge (Change 25) ──
  dailyChallenge: {
    current: null,                  // {id, text, type, points, completedAt}
    history: []
  },

  // ── Weekly Report (Change 22) ──
  weeklyReports: [],                // {id, weekStart, data, pdfPath, createdAt}

  // ── Admin (Change 19) ──
  isAdmin: false,
  adminCode: 'AAKASH-ADMIN-2026'
};
...
async function loadAll() {
  ...
  if (d) {
    ...
    // ── v3 migrations ──
    if (!S.userPhone) S.userPhone = '';
    if (!S.userName) S.userName = '';
    if (!S.userJoinedAt) S.userJoinedAt = '';
    if (!S.connect) S.connect = { bots:[], partnerCode:'', connectedPartner:'', partnerActive:false, partnerMessages:[] };
    if (!S.connect.bots || !S.connect.bots.length) {
      S.connect.bots = [
        { id:1, name:'Arjun', role:'career', expertise:'Career Mentor', chatHistory:[], enabled:true },
        { id:2, name:'Kavya', role:'life', expertise:'Life Coach', chatHistory:[], enabled:true },
        { id:3, name:'Rehan', role:'study', expertise:'Study Buddy', chatHistory:[], enabled:true },
        { id:4, name:'Priya', role:'business', expertise:'Business Strategist', chatHistory:[], enabled:true },
        { id:5, name:'Dev', role:'fitness', expertise:'Fitness Coach', chatHistory:[], enabled:true }
      ];
    }
...
function isDemoMode() { return !(S.apiKeys || []).some(k => k.enabled) && !(typeof MIND !== 'undefined' && MIND.ready); }
...
//  UPDATED: Respectful tone for all sections
// ════════════════════════════════════

const SECTION_CHAT_CONFIG = {
  finance: {
    name: 'Finance Advisor', icon: 'money',
    greeting: 'Namaste! Main aapka financial advisor hoon. Salary, SIP, tax, budget, income, net worth — kuch bhi puchiye.',
    ...
    sysPrompt: `Tu AAKASH hai — MULTI-MILLIONAIRE FINANCIAL ADVISOR.
User ko "aap" se address kar. Respectful aur professional tone.
Data dekh ke specific numbers de. Hinglish mein.
Multi-millionaire mindset se guide kar — wealth building, passive income, compounding.
Har idea ke execution steps clearly batao — kaise start karein, kya invest karein, timeline kya hogi.
Chhoti baat = chhota reply. Income, expenses, net worth, streams — sab track kar.
User ko wealthy banne mein actively help kar — sirf advice nahi, execution plan de.`
  },
  notes: {
    name: 'AI Teacher', icon: 'book',
    greeting: 'Namaste! Main aapka teacher hoon. Koi bhi topic puchiye — simple Hinglish mein samjhaunga. PDF upload karein toh usse bhi padha sakta hoon.',
    quickBtns: ['Kuch samjhao', 'Quiz do', 'Summary', 'Doubt clear'],
    getContext: () => {
      return `\n[NOTES]\nTotal:${S.notes.length}\nRecent:${S.notes.slice(0,10).map(n => n.title).join(', ')||'none'}`;
    },
    sysPrompt: `Tu AAKASH hai — EXPERT TEACHER.
User ko "aap" se address kar. Respectful aur encouraging tone.
Simple Hinglish mein samjhao. Real examples use karo.
Step-by-step explain karo with analogies.
Doubts puchein — "kya yeh clear hua? Koi aur doubt?"
Agar user ne PDF upload kiya hai toh usse reference karke padhao.
Quiz do, test lo, weak areas identify karo.
Goal: user ko genuinely upskill karna hai.`
  },
  habits: {
    name: 'Life Coach', icon: 'habits',
    greeting: 'Namaste! Main aapka life coach hoon. Habits, motivation, health — aapke data ke basis pe advice dunga.',
    ...
    sysPrompt: `Tu AAKASH hai — LIFE COACH.
User ko "aap" se address kar. Strict but motivating aur respectful.
Habit data dekh ke specific advice de. Streak praise kar, miss pe gently push kar.
Accountability maintain kar — pending tasks pe remind kar.
Health, fitness, voice, skin — sab pe guide kar jab puchein.`
  },
  vault: {
    name: 'Productivity Coach', icon: 'target',
    greeting: 'Namaste! Main aapka productivity coach hoon. Tasks, goals, skills, business ideas — aapke data dekh ke advice dunga.',
    ...
    sysPrompt: `Tu AAKASH hai — PRODUCTIVITY COACH.
User ko "aap" se address kar. Respectful aur action-oriented.
Tasks/goals/skills/business ideas dekh ke specific advice de.
Skill-to-money pipeline suggest kar. Actionable steps de.
Execution focus — sirf ideas nahi, kaise implement karein woh batao.`
  }
};
...
window.sendSecChat = async function(section, text) {
  if (isDemoMode()) { showToast('API key add karein Settings mein'); return; }
  const cfg = SECTION_CHAT_CONFIG[section];
  const data = _getSecData(section);
