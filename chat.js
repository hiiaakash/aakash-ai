...
function rChat(ct) {
  ...
  <div style="flex-shrink:0;padding:8px 12px 12px;border-top:1px solid var(--b1);background:var(--c1)">
    ${S.chat.length > 0 ? `<div style="display:flex;gap:4px;margin-bottom:6px;overflow-x:auto">
      <button onclick="exportChatPDF()" style="padding:4px 10px;border-radius:14px;background:var(--c2);border:1px solid var(--b1);color:var(--t3);font-size:10px;font-weight:500;white-space:nowrap;display:flex;align-items:center;gap:3px;cursor:pointer">${I.download} PDF</button>
    </div>` : ''}
...
window.sendMsg = async function(text) {
  ...
  // Streaming callback — update the last assistant bubble live
...
// ── Feature 3: Download any file (image/audio/video) ──
window.downloadFile = function(dataUrl, filename) {
  const a = document.createElement('a');
  a.href = dataUrl;
  a.download = filename || 'aakash-download';
...
// ── Feature 4: Export Chat as PDF ──
window.exportChatPDF = function() {
  if (S.chat.length === 0) { showToast('No chat to export'); return; }
  showToast('Generating PDF...');

  // Build PDF using canvas-based approach (no external lib needed)
  const chatTitle = S.chats.find(c => c.id === S.activeChat)?.title || 'AAKASH AI Chat';
  ...
  container.style.cssText = 'position:fixed;left:-9999px;top:0;width:595px;padding:40px;background:#fff;color:#111;font-family:Inter,system-ui,sans-serif;font-size:12px;line-height:1.7;';

  // Header
  container.innerHTML = `
    <div style="text-align:center;margin-bottom:20px;padding-bottom:15px;border-bottom:2px solid #111">
      <div style="font-size:20px;font-weight:700;letter-spacing:-0.5px">AAKASH AI</div>
      <div style="font-size:10px;color:#888;letter-spacing:2px;margin-top:2px">CHAT EXPORT</div>
      <div style="font-size:11px;color:#555;margin-top:8px">${chatTitle}</div>
      <div style="font-size:10px;color:#888;margin-top:2px">${dateStr}</div>
    </div>
    ${S.chat.filter(m => !m._streaming).map(m => {
      const isUser = m.role === 'user';
      const content = typeof m.content === 'string' ? m.content : '[File content]';
      const cleanContent = content.replace(/[*#`]/g, '').trim();
      return `<div style="margin-bottom:12px;padding:10px 14px;border-radius:10px;background:${isUser?'#f0f0f0':'#fff'};border:1px solid ${isUser?'#e0e0e0':'#ddd'}">
        <div style="font-size:10px;font-weight:600;color:${isUser?'#333':'#111'};margin-bottom:4px">${isUser?'You':'AAKASH'}</div>
        <div style="font-size:11px;color:#333;white-space:pre-wrap;word-wrap:break-word">${cleanContent.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
      </div>`;
    }).join('')}
    <div style="text-align:center;margin-top:20px;padding-top:15px;border-top:1px solid #ddd;font-size:9px;color:#aaa">Generated by AAKASH AI · ${dateStr}</div>
  `;

  document.body.appendChild(container);

  // Use browser print as PDF fallback (most reliable, no lib needed)
  const printWin = window.open('', '_blank', 'width=620,height=800');
  if (!printWin) {
    // Popup blocked — use Blob download with simple text
    document.body.removeChild(container);
    _exportChatAsTextFile(chatTitle, dateStr);
    return;
  }

  printWin.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${chatTitle} - AAKASH AI</title>
    <style>@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');
    *{box-sizing:border-box;margin:0;padding:0}body{font-family:'Inter',system-ui,sans-serif;padding:30px;color:#111;font-size:12px;line-height:1.7;}
    @media print{body{padding:20px;}}</style></head><body>${container.innerHTML}
    <script>setTimeout(()=>{window.print();setTimeout(()=>window.close(),500);},500)<\/script></body></html>`);
  printWin.document.close();
  document.body.removeChild(container);
};

function _exportChatAsTextFile(chatTitle, dateStr) {
  let text = `AAKASH AI — Chat Export\n${chatTitle}\n${dateStr}\n${'═'.repeat(40)}\n\n`;
  S.chat.filter(m => !m._streaming).forEach(m => {
    const content = typeof m.content === 'string' ? m.content : '[File content]';
    text += `${m.role === 'user' ? '👤 You' : '⭐ AAKASH'}:\n${content.replace(/[*#`]/g,'').trim()}\n\n`;
  });
  text += `${'═'.repeat(40)}\nGenerated by AAKASH AI · ${dateStr}`;
  const blob = new Blob([text], { type: 'text/plain;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `aakash-chat-${td()}.txt`;
