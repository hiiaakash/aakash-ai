...
// ── Feature 4: Export Chat as actual PDF file ──
window.exportChatPDF = async function() {
  if (S.chat.length === 0) { showToast('No chat to export'); return; }
  showToast('PDF bana raha hoon...');
  ...
  if (!window.jspdf) {
    try {
      ...
      showToast('PDF library load nahi hui. Internet check karo.');
      return;
    }
  }
  ...
  const pw = 210, ph = 297, mx = 15, mxr = 195, mt = 15;
  let y = mt;
  ...
  doc.text('AAKASH AI', pw / 2, 12, { align: 'center' });
  doc.setFontSize(8); doc.setFont('helvetica', 'normal');
  doc.text('CHAT EXPORT', pw / 2, 18, { align: 'center' });
  ...
  doc.setFontSize(7);
  doc.text(`${chatTitle}  |  ${dateStr}`, pw / 2, 24, { align: 'center' });
  ...
  S.chat.filter(m => !m._streaming).forEach(m => {
    ...
    doc.setTextColor(isUser ? 100 : 17, isUser ? 100 : 17, isUser ? 100 : 17);
    ...
    const lines = doc.splitTextToSize(clean, mxr - mx);

    // Background box
    const boxH = lines.length * 4.2 + 6;
    if (y + boxH > ph - 15) { doc.addPage(); y = mt; }
    doc.setFillColor(isUser ? 240 : 250, isUser ? 240 : 250, isUser ? 240 : 255);
    doc.setDrawColor(isUser ? 220 : 230, isUser ? 220 : 230, isUser ? 220 : 235);
    doc.roundedRect(mx - 2, y, mxr - mx + 4, boxH, 2, 2, 'FD');

    y += 4;
    lines.forEach(line => {
      if (y > ph - 15) { doc.addPage(); y = mt; }
      doc.text(line, mx + 1, y);
      y += 4.2;
    });
    y += 4;
  });
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.text(`AAKASH AI  |  ${dateStr}  |  Page ${i}/${pages}`, pw / 2, ph - 8, { align: 'center' });
  }

  // ── Download ──
  doc.save(`AAKASH-Chat-${td()}.pdf`);
