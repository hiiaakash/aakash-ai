  ...
  S.chat.map((m, idx) => `<div style="display:flex;justify-content:${m.role === 'user' ? 'flex-end' : 'flex-start'};margin-bottom:10px;animation:slideUp .3s">
    ...
    <div style="max-width:82%;position:relative">
      <div style="padding:12px 16px;border-radius:${m.role === 'user' ? '18px 18px 4px 18px' : '4px 18px 18px 18px'};background:${m.role === 'user' ? 'var(--grad)' : 'var(--c1)'};border:${m.role === 'user' ? 'none' : '1px solid var(--b1)'};font-size:15px;line-height:1.7;color:${m.role === 'user' ? '#fff' : 'var(--t1)'};box-shadow:var(--shadow)">
        ...
        ${m.hasGenImage ? `<div style="margin-top:8px"><img src="${m.genImageData}" style="max-width:100%;border-radius:10px;border:1px solid var(--b1)"><div style="display:flex;gap:6px;margin-top:6px"><a href="${m.genImageData}" download="aakash-image-${Date.now()}.png" style="padding:6px 12px;border-radius:6px;background:var(--acBg);color:var(--ac);font-size:12px;font-weight:600;text-decoration:none;border:1px solid var(--acBorder)">💾 Download</a><button onclick="navigator.clipboard.writeText('${(m.genPrompt||'').replace(/'/g,"\\'")}');showToast('Prompt copied!')" style="padding:6px 12px;border-radius:6px;background:var(--c2);border:1px solid var(--b1);color:var(--t3);font-size:12px;font-weight:500;cursor:pointer">📋 Prompt</button></div></div>` : ''}
      </div>
      <!-- Message Actions -->
...
window.sendMsg = async function(text) {
  ...
  let apiMessages = S.chat.slice(-20).map(m => {
    if (m === userMsg && pendingFile) {
      if (pendingFile.type === 'image') {
        ...
        // Office files — extract what we can, tell AI about the file
        return { role: 'user', content: `[Uploaded file: ${_escHtml(pendingFile.name)} (${pendingFile.ext} format)]\n\nFile content is in ${pendingFile.ext} format. Please analyze based on the filename and user's question.\n\n${content || 'Is file ko analyze karo aur important points batao.'}` };
      } else {
  ...
  // Check if AI response contains [GENERATE_IMAGE: ...] tag
  const imgMatch = reply.match(/\[GENERATE_IMAGE:\s*(.+?)\]/i);
  if (imgMatch && hasCap('image_gen')) {
    // AI wants to generate an image and we have the capability
    const imgPrompt = imgMatch[1].trim();
    const textPart = reply.replace(/\[GENERATE_IMAGE:\s*.+?\]/i, '').trim();
    if (textPart) S.chat.push({ role: 'assistant', content: textPart });
    
    // Show generating state
    S.chat.push({ role: 'assistant', content: '🎨 Image generate ho raha hai...', isGenerating: true });
    rChat(document.getElementById('ct'));
    
    // Generate image
    const imgResult = await generateImage(imgPrompt);
    // Remove generating message
    S.chat = S.chat.filter(m => !m.isGenerating);
    
    if (imgResult.ok && imgResult.images?.length) {
      const imgData = imgResult.images[0];
      S.chat.push({ role: 'assistant', content: `🎨 Image ready!`, hasGenImage: true, genImageData: `data:${imgData.mime};base64,${imgData.data}`, genPrompt: imgPrompt });
    } else {
      S.chat.push({ role: 'assistant', content: `⚠️ Image generate nahi ho payi: ${imgResult.error || 'Unknown error'}. Dobara try karo.` });
    }
  } else {
