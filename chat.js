...
window.sendMsg = async function(text) {
  ...
  // ── URL/Link Reader — fetch, store, save notes, PDF, short summary to AI ──
  let urlContext = '';
  ...
  if (detectedUrls.length > 0) {
    ...
    // Store full content — NO auto-PDF, NO auto-save
    const good = fetchResults.filter(f => f.ok);
    good.forEach(f => storeWebContent(f));
    if (good.length) showToast(`${good.length} link read ho gayi — ${good.reduce((s,f)=>s+f.charCount,0).toLocaleString()} chars`);

    urlContext = buildUrlContext(fetchResults);
    const failed = fetchResults.filter(f => !f.ok);
    if (failed.length && !good.length) showToast(`Link read nahi ho payi: ${failed[0].error}`);
  }
  // ── Follow-up Q&A about previously fetched content ──
  else if (isFollowUpAboutWeb(content)) {
    urlContext = buildQAContext(content);
  }
  ...
  // ── Web Reader commands — PDF, Save, etc. ──
  let cleanReply = reply;
  if (reply.includes('[MAKE_PDF]')) {
    cleanReply = reply.replace(/\[MAKE_PDF\]/g, '').trim();
    setTimeout(() => showPDFNamePrompt(), 500);
  }
  if (reply.includes('[SAVE_NOTES]')) {
    cleanReply = cleanReply.replace(/\[SAVE_NOTES\]/g, '').trim();
    saveWebTo('notes');
  }
  if (reply.includes('[SAVE_VAULT]')) {
    cleanReply = cleanReply.replace(/\[SAVE_VAULT\]/g, '').trim();
    saveWebTo('vault');
  }
  if (reply.includes('[SAVE_TASK]')) {
    cleanReply = cleanReply.replace(/\[SAVE_TASK\]/g, '').trim();
    saveWebTo('task');
  }
  if (reply.includes('[DELETE_NOTES]')) {
    cleanReply = cleanReply.replace(/\[DELETE_NOTES\]/g, '').trim();
    deleteWebFrom('notes');
  }
  if (reply.includes('[DELETE_VAULT]')) {
    cleanReply = cleanReply.replace(/\[DELETE_VAULT\]/g, '').trim();
    deleteWebFrom('vault');
  }
...
window.downloadFile = function(dataUrl, filename) {
  ...
  const newName = prompt('File name set karo:', defaultName);
...
// ── Feature 4: Export Chat as PDF — PURE JS, ZERO CDN, FULLY OFFLINE ──
window.exportChatPDF = function() {
  ...
  // Build pages content
  const pageW = 595, pageH = 842, mx = 50, maxW = 495, lineH = 14, fontSize = 10, smallFont = 8;
  ...
  function wrapText(text, maxChars) {
    ...
    words.forEach(w => {
      if ((line + ' ' + w).length > maxChars) { if (line) lines.push(line); line = w; }
  ...
  function addText(text, x, size, bold, r, g, b) {
    const font = bold ? '/F2' : '/F1';
    currentStream += `BT ${font} ${size} Tf ${r||0} ${g||0} ${b||0} rg ${x} ${cy} Td (${escPdf(text)}) Tj ET\n`;
  }
  ...
  // Chat title
  addText(chatTitle, mx, 9, false, 0.4, 0.4, 0.4);
  cy -= 20;
  ...
  S.chat.filter(m => !m._streaming).forEach(m => {
    ...
    // Label
    addText(label, mx, smallFont, true, isUser ? 0.4 : 0.07, isUser ? 0.4 : 0.07, isUser ? 0.4 : 0.07);
    cy -= lineH;

    // Message lines
    lines.forEach(line => {
      checkSpace(lineH);
      addText(line, mx + 5, fontSize, false, 0.2, 0.2, 0.2);
      cy -= lineH;
    });

    cy -= 8; // gap between messages
  });

  // Push last page
  if (currentStream) pages.push(currentStream);

  // ── Build PDF objects for each page ──
  const pageObjIds = [];
  ...
  const fileName = prompt('File name set karo:', defaultName);
