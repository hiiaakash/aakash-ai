...
const MIND = {
  ...
  async learn(question, answer, category) {
    ...
    if (answer.startsWith('API Error') || answer.includes('fail ho gayi')) return; // don't learn errors
    ...
    if (typeof FIRE !== 'undefined' && FIRE.ready) {
      ...
      FIRE.save('memory', docId, { question: question.slice(0, 500), answer: answer.slice(0, 3000), keywords, category: category || this._categorize(question), timestamp: Date.now(), useCount: 0, rating: 0 });
    }
...
- Kabhi formal/robotic mat ho. Real friend jaisa bol.`;
  ...
  async decide(text) {
    ...
    const offline = this.tryOffline(clean);
    if (offline) {
      return { answer: offline, source: 'offline', confidence: 1.0 };
    }

    // Step 2: Try memory recall (instant, free)
    const memory = await this.recall(clean);
    if (memory && memory.score >= 0.45) {
      return { answer: memory.answer, source: 'memory', confidence: memory.score };
    }

    // Step 3: Need API — but we can optimize context
  ...
  async deepLearn(question, answer) {
    ...
    // Extract facts from answer (simple heuristic)
    const facts = answer.match(/(?:[\w\s]+) (?:hai|hota hai|hota|hain|means|is called|matlab|ka matlab)/gi);
    if (facts) {
